...
###############################################################################
# MOCK FUNCTIONS USING TEST FRAMEWORK
###############################################################################
...
# Mock qm command
cat >"$TEST_TEMP_DIR/bin/qm" <<'MOCK_QM'
#!/bin/bash
case "$1" in
    config)
        echo "memory: 2048"
...
MOCK_QM
chmod +x "$TEST_TEMP_DIR/bin/qm"

# Mock pct command
cat >"$TEST_TEMP_DIR/bin/pct" <<'MOCK_PCT'
#!/bin/bash
case "$1" in
    config)
        vmid="$2"
        if (( vmid >= 200 && vmid <= 210 )); then
            echo "memory: 1024"
            echo "cores: 2"
            echo "hostname: ct${vmid}"
            exit 0
        else
            exit 1
        fi
