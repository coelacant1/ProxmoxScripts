...
#
# Usage:
#   ./AddStorage.sh <storage_type> <storage_id> <server> <export_path|share> [options]
#
# Arguments:
...
#   --username <user>    - Username for SMB/CIFS or PBS authentication
#   --password <pass>    - Password for SMB/CIFS or PBS authentication
#   --domain <domain>    - Domain for SMB/CIFS authentication
#   --fingerprint <fp>   - SSL fingerprint for PBS
...
#   ./AddStorage.sh pbs PBS-Backup 192.168.1.50 --username backup@pbs --password secret --datastore main
#
# Function Index:
#   - usage
#   - parse_args
...
Optional Arguments:
  --content <types>      - Content types (comma-separated)
                          Default: vztmpl,backup,iso,snippets (NFS/SMB)
                                   backup (PBS)
  --username <user>      - Username (SMB/CIFS/PBS)
  --password <pass>      - Password (SMB/CIFS/PBS)
  --domain <domain>      - Domain (SMB/CIFS only)
  --fingerprint <fp>     - SSL fingerprint (PBS only)
...
Examples:
  ...
  ${0##*/} pbs PBS-Backup 192.168.1.50 --username backup@pbs --password secret --datastore main
...
add_pbs_storage() {
    ...
    [[ -n "$FINGERPRINT" ]] && __info__ "  Fingerprint: ${FINGERPRINT}"
    [[ -n "$NODES" ]] && __info__ "  Nodes: ${NODES}"
