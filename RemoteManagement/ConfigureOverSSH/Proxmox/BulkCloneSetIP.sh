...
for ((i=0; i<instanceCount; i++)); do
  ...
  echo "Trying QEMU Guest Agent to get IP for VM $currentVmId ..."
  agentIp=""
  if agentIp="$(get_ip_via_agent "$currentVmId" 2>/dev/null)"; then
    echo "Guest Agent reported IP: $agentIp"
  else
    echo "Guest Agent IP not available; using template-IP reconfig path..."
  fi

  # If we didn't get an IP from agent, fall back to your existing approach:
  if [[ -z "$agentIp" ]]; then
    echo "Connecting to template IP ($templateIpAddr) to rewrite /etc/network/interfaces..."
...
EOF
    ...
    echo "Waiting for SSH on new IP: $currentIp ..."
    __wait_for_ssh__ "$currentIp" "$sshUsername" "$sshPassword"
    agentIp="$currentIp"
  else
    # Ensure SSH is up on the agent IP
    echo "Waiting for SSH on agent IP: $agentIp ..."
    __wait_for_ssh__ "$agentIp" "$sshUsername" "$sshPassword"
  fi
...
INNER_EOF
  ...
  echo "Copying inner MAC-update script to $agentIp ..."
  scp_child "$tmpInner" "/root/_mac_update.sh" "$agentIp"
  rm -f "$tmpInner"

  echo "Applying MAC prefix + rewriting child VMs on $agentIp ..."
  ssh_child "$agentIp" "bash /root/_mac_update.sh '$newPrefix'"

  echo "Verification on $agentIp:"
  ssh_child "$agentIp" "grep -H '^mac_prefix' /etc/pve/datacenter.cfg || true"
  ssh_child "$agentIp" "qm list || true; pct list || true"

  # Next IP
  ipInt=$((ipInt + 1))
  echo "----- Completed VM $currentVmId ($agentIp) -----"
done
