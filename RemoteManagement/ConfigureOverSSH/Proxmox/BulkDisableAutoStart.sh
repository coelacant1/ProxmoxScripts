...
disable_autostart_inner() {
  ...
  sshpass -p "$sshPass" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null \
    "$sshUser@$host" "echo '$sshPass' | sudo -S -p '' bash -s" <<'INNER'
  # Run inner commands via sudo so non-root remote accounts (that have sudo)
  # can perform Proxmox management tasks. We pipe the password to sudo (-S).
